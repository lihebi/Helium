...
$Header: /mnt/leo2/cvs/sabo/hist-040105/bind/b2/sig-bad.c,v 1.2 2004/02/04 20:14:32 tleek Exp $
...
rrextract(u_char *msg, int msglen, u_char *rrp, u_char *dname, int namelen)
{
	...
	case T_SIG: {
		...
		/* 
		   n is a signed int... if your compiler casts sizeof(data) 
		   to a signed int, then the following if-condition will be
		   false for a negative n... later on n will be used as a
		   size argument in memcpy and will get cast to a very large
		   positive number, resulting in an overflow.
		   it should be noted that some compilers will not cast
		   sizeof to a signed int, but rather they will cast n to
		   an unsigned int, in which case the check below will detect
		   negative n's
		*/
		...
		/* TRL: NOTE: in order to simulate Sparc compiler had to force a cast to (int) here */
